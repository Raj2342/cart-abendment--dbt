...
session_metrics AS (
    ...
    SELECT 
        ...
        -- JARVIS FIX: Switched to Athena's date_diff logic
        ROUND(CAST(date_diff('second', MIN(event_time), MAX(event_time)) AS DOUBLE) / 60.0, 2) AS duration_minutes,
...
valid_human_sessions AS (
    ...
    WHERE total_events > 1             -- KILL THE GHOSTS
      ...
      AND (
          ...
          OR 
        -- JARVIS FIX: Switched to Athena's date_diff logic
        (first_cart_time IS NOT NULL AND date_diff('second', first_cart_time, first_purchase_time) >= 10)
      )
)
