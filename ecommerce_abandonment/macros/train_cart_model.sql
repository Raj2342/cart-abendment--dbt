...
{{ log("Igniting Multiclass XGBoost Engine in BigQuery...", info=True) }}

{% set train_query %}
    CREATE OR REPLACE MODEL `{{ target.project }}.ecommerce_analytics.triage_model`
    OPTIONS(
        model_type = 'BOOSTED_TREE_CLASSIFIER',
        input_label_cols = ['target_label'], -- JARVIS UPDATE: Our new 3-class target
    ...
    SELECT
        * EXCEPT(
            user_session, 
            user_id, 
            has_purchase, -- CRITICAL: Model cannot see the future (Data Leakage prevention)
            has_cart,     -- Excluded: Not a predictive feature
            has_view      -- Excluded: Not a predictive feature
        ) 
    FROM
        {{ ref('model_features') }}; -- JARVIS UPDATE: Pointing to the ML specific table
{% endset %}
